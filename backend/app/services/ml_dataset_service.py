...
def _build_features_for_dog_day(
    ...
    worked_days_3d = len([w for w in logs_3d if w.worked])
...
def build_ml_dataset(
    ...
    for dog in dogs:
        ...
        for as_of_date in unique_dates:
            ...
            risk = _evaluate_risk_from_features(features)

            features["target_risk_level"] = risk.risk_level
            features["target_usage_level"] = risk.usage_level
            features["target_flag_count"] = len(risk.flags)
            features["target_flags"] = "|".join(risk.flags)
